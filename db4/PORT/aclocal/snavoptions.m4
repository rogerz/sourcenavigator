# Copy from dist/aclocal/options.m4 and munged for SNav to allow db4 to be built in 1.85
# compatibility mode by default (along with other changes to the default settings)
...
AC_MSG_CHECKING(if --enable-cryptography option specified)
AC_ARG_ENABLE(cryptography,
	[AC_HELP_STRING([--enable-cryptography],
	    [Build database cryptography support.])],
	[db_cv_build_cryptography="$enable_cryptography"], [db_cv_build_cryptography="no"])
...
AC_MSG_CHECKING(if --enable-partition option specified)
AC_ARG_ENABLE(partition,
	[AC_HELP_STRING([--enable-partition],
	    [Build partitioned database support.])],
	[db_cv_build_partition="$enable_partition"], [db_cv_build_partition="no"])
AC_MSG_RESULT($db_cv_build_partition)

AC_MSG_CHECKING(if --enable-compression option specified)
AC_ARG_ENABLE(compression,
	[AC_HELP_STRING([--enable-compression],
	    [Build compression support.])],
	[db_cv_build_compression="$enable_compression"], [db_cv_build_compression="no"])
AC_MSG_RESULT($db_cv_build_compression)
...
AC_MSG_CHECKING(if --enable-queue option specified)
AC_ARG_ENABLE(queue,
	[AC_HELP_STRING([--enable-queue],
	    [Build Queue access method.])],
	[db_cv_build_queue="$enable-queue"], [db_cv_build_queue="no"])
AC_MSG_RESULT($db_cv_build_queue)

AC_MSG_CHECKING(if --enable-replication option specified)
AC_ARG_ENABLE(replication,
	[AC_HELP_STRING([--enable-replication],
	    [Build database replication support.])],
	[db_cv_build_replication="$enable_replication"], [db_cv_build_replication="no"])
AC_MSG_RESULT($db_cv_build_replication)

AC_MSG_CHECKING(if --enable-statistics option specified)
AC_ARG_ENABLE(statistics,
	[AC_HELP_STRING([--enable-statistics],
	    [Build statistics support.])],
	[db_cv_build_statistics="$enable_statistics"], [db_cv_build_statistics="no"])
AC_MSG_RESULT($db_cv_build_statistics)

AC_MSG_CHECKING(if --enable-verify option specified)
AC_ARG_ENABLE(verify,
	[AC_HELP_STRING([--enable-verify],
	    [Build database verification support.])],
	[db_cv_build_verify="$enable_verify"], [db_cv_build_verify="no"])
AC_MSG_RESULT($db_cv_build_verify)

AC_MSG_CHECKING(if --disable-compat185 option specified)
AC_ARG_ENABLE(compat185,
	AC_HELP_STRING([--disable-compat185],
			[Do not build DB 1.85 compatibility API.]),, enableval="yes")
db_cv_compat185="$enableval"
case "$enableval" in
 no) AC_MSG_RESULT(yes);;
yes) AC_MSG_RESULT(no);;
esac
